...
class TestColumnFormatter:
    ...
    def test_custom_function_formatting(self):
        ...
        df = pd.DataFrame(
            {"revenue": [1000.50, 2500.75, 3750.25], "sessions": [100, 250, 500]}
        )

        formatter = ColumnFormatter(
            {"revenue": lambda x: f"${x:,.2f}", "sessions": lambda x: f"{x:,}"}
        )
    ...
    def test_percentage_helper_basic(self):
        ...
        df = pd.DataFrame({"deviation_pct": [15.3, 25.7, 5.1]})
    ...
    def test_percentage_helper_multiple_columns(self):
        ...
        df = pd.DataFrame({"growth_rate": [15.3, 25.7], "change_pct": [10.5, 20.2]})

        formatter = ColumnFormatter.percentage(
            ["growth_rate", "change_pct"], decimal_places=2
        )
        result = formatter.format(df)
    ...
    def test_percentage_helper_with_multiplication(self):
        ...
        df = pd.DataFrame({"conversion_rate": [0.153, 0.257]})  # Values between 0-1

        formatter = ColumnFormatter.percentage(
            "conversion_rate", decimal_places=1, multiply_by_100=True
        )
        result = formatter.format(df)
    ...
    def test_percentage_helper_decimal_places(self):
        ...
        df = pd.DataFrame({"value": [15.3456, 25.7891]})
    ...
    def test_missing_column(self):
        ...
        formatter = ColumnFormatter(
            {"missing_column": lambda x: f"{x}%", "value": lambda x: f"${x}"}
        )
        result = formatter.format(df)
    ...
    def test_preserve_original_dataframe(self):
        ...
        original_df = pd.DataFrame({"value": [100, 200, 300]})
    ...
    def test_multiple_formatters_on_same_column(self):
        ...
        df = pd.DataFrame({"value": [100.5, 200.7]})

        # Create two formatters and apply sequentially
        formatter1 = ColumnFormatter({"value": lambda x: f"${x}"})
        formatter2 = ColumnFormatter({"value": lambda x: f"{x}%"})
