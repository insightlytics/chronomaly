...
class TestPivotTransformer:
    ...
    def test_basic_pivot(self):
        ...
        transformer = PivotTransformer(index="date", columns="product", values="sales")
    ...
    def test_pivot_with_datetime_index_not_named_date(self):
        ...
        transformer = PivotTransformer(
            index="timestamp", columns="product", values="sales"
        )
        result = transformer.pivot_table(df)
        ...
        assert (
            result.index.freq is not None
        ), "Bug #4: Frequency not set for datetime index named 'timestamp'"
