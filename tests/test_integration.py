...
try:
    ...
    workflow = NotificationWorkflow(anomalies_data=anomalies_df, notifiers=[notifier])
...
try:
    critical_notifier = EmailNotifier(
        to=["oncall@example.com"], subject="🔴 CRITICAL - {date}"
    )

    team_notifier = EmailNotifier(
        to=["team@example.com"], subject="Daily Summary {date:%d.%m.%Y}"
    )

    workflow = NotificationWorkflow(
        anomalies_data=anomalies_df, notifiers=[critical_notifier, team_notifier]
    )
